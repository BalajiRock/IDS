...
consumer = KafkaConsumer("Tcp",bootstrap_servers=['localhost:9092'])
DestinationPort = 0  
SourcePort = 0
SourceIp = ""
DestinationIP = "" 
...
for packet in consumer:
    ...
    for i in range(lenOfLines):
        ...
        elif(lines[i].startswith("[Protocols in frame:")):
            if("ftp" in lines[i]):
                FTPFlag = True 
            elif("ssh" in lines[i]):
                SSHFlag = True       
    ...
    if(key1 not in hashTable.keys() and key2 not in hashTable.keys()):
        value = [1,0,packetLength,0,packetLength,packetLength,0,0,TimeStamp,TimeStamp,0,Rev_Flag,CWE_Flag,ECE_Flag,URG_Flag,ACK_Flag,PSH_Flag,RST_Flag,SYN_Flag,FIN_Flag,False,False]
        ...
        if(key1 in hashTable.keys()):
            ...
            val[18] += SYN_Flag
            val[19] += FIN_Flag  
            val[20] = FTPFlag or val[20] 
            val[21] = SSHFlag or val[21]     
    ...
    if FIN_Flag:
        ...
        if key1 in hashTable.keys():
            ...
            # file.write(f"{TimeStamp},{SourceIp},{DestinationIP},{SourcePort},{DestinationPort},{Protocol},{values[0]},{values[1]},{values[2]},{values[3]},{values[4]},{values[5]},{values[6]},{values[7]},{fwMean},{bwMean},{subtract_time(values[8],TimeStamp)},{values[10]},{values[10]/(values[0]+values[1])},{values[11]},{values[12]},{values[13]},{values[14]},{values[15]},{values[16]},{values[17]},{values[18]},{values[19]},{values[20]},{values[21]},{label}\n")

            print(TimeStamp,values[8])    
            print("Flow Duration : ",subtract_time(values[8],TimeStamp))
            
            finalData = f"{TimeStamp},{SourceIp},{DestinationIP},{SourcePort},{DestinationPort},{Protocol},{values[0]},{values[1]},{values[2]},{values[3]},{values[4]},{values[5]},{values[6]},{values[7]},{fwMean},{bwMean},{subtract_time(values[8],TimeStamp)},{values[10]},{values[10]/(values[0]+values[1])},{values[11]},{values[12]},{values[13]},{values[14]},{values[15]},{values[16]},{values[17]},{values[18]},{values[19]},{values[20]},{values[21]}\n"
            producer.send("finalData",bytes(str(finalData), 'utf-8'))
            hashTable.pop(key1)
        else:
...
# TimeStamp,SourceIp,DestinationIP,SourcePort,DestinationPort,Protocol,No_of_forward_packets,No_of_backward_packets,total_forward_packets_length,total_backward_packets_length,Max_forward_packet_length,Min_forward_packet_length,Max_backward_packet_length,Min_backward_packet_length,Mean_forward_packet_length,Mean_backward_packet_lenght,Flow_duration,Inter_arrival_time,Mean_inter_arrival_time,Rev_Flag,CWE_Flag,ECE_Flag,URG_Flag,ACK_Flag,PSH_Flag,RST_Flag,SYN_Flag,FIN_Flag
