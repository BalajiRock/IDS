...
consumer = KafkaConsumer("Udp",bootstrap_servers=['localhost:9092'])
DestinationPort = 0  
SourcePort = 0
SourceIp = ""
DestinationIP = "" 
            ...
            if("Reserved:" in lines[i+1]):
                while(not lines[i].strip().startswith("Window: ")):
                    if("Reserved:" in lines[i] and lines[i].split(":")[1].strip() =="Set"):
                        ...
                        SYN_Flag = 1   
                        noOfPackets = random.randint(5,10)
